...
//demo -spring security -Authentication flow
public class AuthenticationSample {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			SecurityContextHolder.getContext().setAuthentication(result);//sets the Authentication(principal)in context
...
//authenticationManager utility -
class AuthenticationManagerImpl implements AuthenticationManager {

	public UserDetailsService userDetailsService() {
		InMemoryUserDetailsManager manager = new InMemoryUserDetailsManager();
		manager.createUser(User.withUsername("deependra").password("password").roles("ADMIN").build());
	...
	public Authentication authenticate(Authentication auth) throws AuthenticationException {
		...
		if(user !=null){
			if(user.getPassword().equals(auth.getCredentials())){
				if(user.getUsername().equals("deependra")){
