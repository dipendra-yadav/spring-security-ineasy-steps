...
public class LoginController {
	...
	public ModelAndView showLogin(Model model) {
		...
		if (!model.containsAttribute("user")) {
			mav.addObject("user", user);//bind the model -command object
		} else
			mav.addObject(model);
		return mav;
	}
	...
	public String loginPeople(@Valid @ModelAttribute("user") User user, BindingResult result, RedirectAttributes attr,HttpServletRequest request) {
		...
		if (s.equalsIgnoreCase("Successful")) { //put user info into  Httpsession
	...
	public String logout(HttpServletRequest request) {
		HttpSession session = request.getSession(false);//acquire old session and invalidate.
		session.invalidate();
		return "redirect:/";
	}
